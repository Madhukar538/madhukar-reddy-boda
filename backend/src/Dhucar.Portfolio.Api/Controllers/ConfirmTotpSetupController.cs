 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Confirms the authenticator app and returns recovery codes.
 */
[Route("api/[controller]")]
[ApiController]
[EnableRateLimiting("auth")]
public class ConfirmTotpSetupController(AuthBAL objAuthBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   ConfirmTotpSetup
    // Method Description    :   Confirms the authenticator app and returns recovery codes.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>ConfirmTotpSetup : </c> Confirms the authenticator app and returns recovery codes.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> ConfirmTotpSetup([FromBody] ConfirmTotpSetupRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objAuthBAL.ConfirmTotpSetup(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(ConfirmTotpSetup)}", string.Empty, $"Exception in {nameof(ConfirmTotpSetup)} Method");
        }
        stopwatch.Stop();
