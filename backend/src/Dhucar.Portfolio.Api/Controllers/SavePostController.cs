...
public class SavePostController(PostBAL objPostBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   SavePost
    // Method Description    :   Creates or updates a post.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>SavePost : </c> Creates or updates a post.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> SavePost([FromBody] SavePostRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objPostBAL.SavePost(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(SavePost)}", string.Empty, $"Exception in {nameof(SavePost)} Method");
        }
        stopwatch.Stop();
