 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Removes a passkey after a fresh authenticator code.
 */
[Route("api/[controller]")]
[ApiController]
[JwtAuthentication]
[EnableRateLimiting("admin")]
public class DeletePasskeyController(PasskeyBAL objPasskeyBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    ...
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> DeletePasskey([FromBody] DeletePasskeyRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objPasskeyBAL.DeletePasskey(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(DeletePasskey)}", string.Empty, $"Exception in {nameof(DeletePasskey)} Method");
        }
        stopwatch.Stop();
