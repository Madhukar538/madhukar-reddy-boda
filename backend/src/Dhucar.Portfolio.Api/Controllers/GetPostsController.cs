...
public class GetPostsController(PostBAL objPostBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetPosts
    // Method Description    :   Lists published posts.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetPosts : </c> Lists published posts.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetPosts()
    {
        ...
        try
        {
            objResponse = await objPostBAL.GetPosts(false);
        }
        catch (Exception ex)
        {
            objResponse.ReturnCode = (int)ErrorCode.TechnicalError;
            objResponse.ReturnMessage = "Technical Error.";
            codeLog.Error(ex, $"Step {nameof(GetPosts)}", string.Empty, $"Exception in {nameof(GetPosts)} Method");
        }
        stopwatch.Stop();
