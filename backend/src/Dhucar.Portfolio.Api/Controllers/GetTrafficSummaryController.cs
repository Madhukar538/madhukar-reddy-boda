 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Returns the traffic dashboard summary.
 */
[Route("api/[controller]")]
[ApiController]
[JwtAuthentication]
[EnableRateLimiting("admin")]
public class GetTrafficSummaryController(TrafficBAL objTrafficBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetTrafficSummary
    // Method Description    :   Returns the traffic dashboard summary.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetTrafficSummary : </c> Returns the traffic dashboard summary.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetTrafficSummary([FromBody] GetTrafficSummaryRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objTrafficBAL.GetTrafficSummary(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(GetTrafficSummary)}", string.Empty, $"Exception in {nameof(GetTrafficSummary)} Method");
        }
        stopwatch.Stop();
