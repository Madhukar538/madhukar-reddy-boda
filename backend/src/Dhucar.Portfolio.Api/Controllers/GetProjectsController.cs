 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Lists published projects.
 */
[Route("api/[controller]")]
[ApiController]
[EnableRateLimiting("public")]
public class GetProjectsController(ProjectBAL objProjectBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetProjects
    // Method Description    :   Lists published projects.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetProjects : </c> Lists published projects.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetProjects([FromBody] GetProjectsRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objProjectBAL.GetProjects(objAPIRequest, false);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(GetProjects)}", string.Empty, $"Exception in {nameof(GetProjects)} Method");
        }
        stopwatch.Stop();
