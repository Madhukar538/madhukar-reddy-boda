 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Tells the sign-in page which sign-in methods are on.
 */
[Route("api/[controller]")]
[ApiController]
[EnableRateLimiting("public")]
public class GetSignInOptionsController(AuthBAL objAuthBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetSignInOptions
    // Method Description    :   Tells the sign-in page which sign-in methods are on.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetSignInOptions : </c> Tells the sign-in page which sign-in methods are on.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetSignInOptions()
    {
        Stopwatch stopwatch = Stopwatch.StartNew();
        Response<object> objResponse = new Response<object>();
        try
        {
            objResponse = await objAuthBAL.GetSignInOptions();
        }
        catch (Exception ex)
        {
            objResponse.ReturnCode = (int)ErrorCode.TechnicalError;
            objResponse.ReturnMessage = "Technical Error.";
            codeLog.Error(ex, $"Step {nameof(GetSignInOptions)}", string.Empty, $"Exception in {nameof(GetSignInOptions)} Method");
        }
        stopwatch.Stop();
