 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Signs out this session or every session.
 */
[Route("api/[controller]")]
[ApiController]
[JwtAuthentication]
[EnableRateLimiting("admin")]
public class LogoutController(SessionBAL objSessionBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   Logout
    // Method Description    :   Signs out this session or every session.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>Logout : </c> Signs out this session or every session.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> Logout([FromBody] LogoutRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objSessionBAL.EndSession(objAPIRequest.IsAllSessions);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(Logout)}", string.Empty, $"Exception in {nameof(Logout)} Method");
        }
        stopwatch.Stop();
