 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Lists every post including drafts.
 */
[Route("api/[controller]")]
[ApiController]
[JwtAuthentication]
[EnableRateLimiting("admin")]
public class GetAllPostsController(PostBAL objPostBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetAllPosts
    // Method Description    :   Lists every post including drafts.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetAllPosts : </c> Lists every post including drafts.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetAllPosts()
    {
        ...
        try
        {
            objResponse = await objPostBAL.GetPosts(true);
        }
        catch (Exception ex)
        {
            objResponse.ReturnCode = (int)ErrorCode.TechnicalError;
            objResponse.ReturnMessage = "Technical Error.";
            codeLog.Error(ex, $"Step {nameof(GetAllPosts)}", string.Empty, $"Exception in {nameof(GetAllPosts)} Method");
        }
        stopwatch.Stop();
