 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Returns one published post.
 */
[Route("api/[controller]")]
[ApiController]
[EnableRateLimiting("public")]
public class GetPostController(PostBAL objPostBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   GetPost
    // Method Description    :   Returns one published post.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>GetPost : </c> Returns one published post.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> GetPost([FromBody] GetPostRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objPostBAL.GetPost(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(GetPost)}", string.Empty, $"Exception in {nameof(GetPost)} Method");
        }
        stopwatch.Stop();
