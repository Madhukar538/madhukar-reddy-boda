...
public class SaveProjectController(ProjectBAL objProjectBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   SaveProject
    // Method Description    :   Creates or updates a project.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>SaveProject : </c> Creates or updates a project.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> SaveProject([FromBody] SaveProjectRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objProjectBAL.SaveProject(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(SaveProject)}", string.Empty, $"Exception in {nameof(SaveProject)} Method");
        }
        stopwatch.Stop();
