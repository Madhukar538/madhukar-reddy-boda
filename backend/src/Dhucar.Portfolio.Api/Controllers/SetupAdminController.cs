 ...
 * Layer           :  Controller
 * Modified By     :
 * Description     :  Creates the admin account with the one-time setup token and starts TOTP enrolment.
 */
[Route("api/[controller]")]
[ApiController]
[EnableRateLimiting("auth")]
public class SetupAdminController(AuthBAL objAuthBAL, ICodeLogger codeLog) : ControllerBase
{
    //****************************************************************************************************
    // Layer                 :   Controller
    // Method Name           :   SetupAdmin
    // Method Description    :   Creates the admin account with the one-time setup token and starts TOTP enrolment.
    ...
    //****************************************************************************************************
    /// <summary>
    /// <c>SetupAdmin : </c> Creates the admin account with the one-time setup token and starts TOTP enrolment.
    /// </summary>
    [HttpPost]
    public async Task<IActionResult> SetupAdmin([FromBody] SetupAdminRequestDTO objAPIRequest)
    {
        ...
        try
        {
            ...
            {
                objResponse = await objAuthBAL.SetupAdmin(objAPIRequest);
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            codeLog.Error(ex, $"Step {nameof(SetupAdmin)}", string.Empty, $"Exception in {nameof(SetupAdmin)} Method");
        }
        stopwatch.Stop();
